...
function PatientForm({
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="text-xs text-muted-foreground mb-1 block">Alergias conocidas</label>
        <Textarea value={form.allergies} onChange={(e) => set("allergies", e.target.value)} placeholder="Penicilina, látex, etc." rows={2} />
      </div>
      <div>
        <label className="text-xs text-muted-foreground mb-1 block">Notas médicas generales</label>
        <Textarea value={form.medicalNotes} onChange={(e) => set("medicalNotes", e.target.value)} placeholder="Antecedentes, condiciones crónicas, etc." rows={3} />
      </div>
      <DialogFooter>
...
function PatientDetailPanel({
  ...
  const uploadFile = trpc.medical.uploadFile.useMutation({
    onSuccess: (data) => {
      // Agregar URL al último expediente o al campo de attachments del form
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end bg-black/50" onClick={onClose}>
      <div
        className="h-full w-full max-w-2xl bg-[#0f1117] border-l border-white/10 overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 z-10 bg-[#0f1117] border-b border-white/10 px-6 pt-6 pb-4">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-full bg-[#FF6B00]/20 flex items-center justify-center text-xl font-bold text-[#FF6B00] overflow-hidden">
                {patient.photoUrl
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
            <TabsList className="bg-white/5">
              ...
              <TabsTrigger value="records">Expediente ({records.length})</TabsTrigger>
              <TabsTrigger value="files">Archivos ({allAttachments.length})</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { label: "Fecha de nacimiento", value: patient.birthDate ? new Date(patient.birthDate).toLocaleDateString("es-MX") : null },
              ...
              {patient.allergies && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                  ...
                  <div className="text-sm text-white">{patient.allergies}</div>
                </div>
              )}
              {patient.medicalNotes && (
                <div className="bg-white/5 rounded-lg p-3">
                  <div className="text-xs text-muted-foreground mb-1">Notas médicas</div>
                  <div className="text-sm text-white whitespace-pre-wrap">{patient.medicalNotes}</div>
                </div>
              )}
              <div className="pt-2">
                <Button
                    ...
                    <div key={appt.id} className="bg-white/5 rounded-lg p-4 border border-white/10">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          {appt.notes && <div className="text-sm text-white/70 mt-2">{appt.notes}</div>}
                        </div>
          ...
          {activeTab === "records" && (
            <div className="space-y-3">
              ...
              {records.length === 0 ? (
                ...
                records.map((rec) => (
                  <div key={rec.id} className="bg-white/5 rounded-lg p-4 border border-white/10 space-y-2">
                    ...
                    {rec.diagnosis && (
                      <div>
                        ...
                        <div className="text-sm text-white">{rec.diagnosis}</div>
                      </div>
                    )}
                    {rec.treatment && (
                      <div>
                        <div className="text-xs text-blue-400 font-semibold mb-1">Tratamiento</div>
                        <div className="text-sm text-white">{rec.treatment}</div>
                      </div>
                    )}
                    {rec.prescription && (
                      <div>
                        <div className="text-xs text-green-400 font-semibold mb-1">Prescripción</div>
                        <div className="text-sm text-white">{rec.prescription}</div>
                      </div>
                    )}
                    {rec.clinicalNotes && (
                      <div>
                        <div className="text-xs text-muted-foreground font-semibold mb-1">Notas clínicas</div>
                        <div className="text-sm text-white/80 whitespace-pre-wrap">{rec.clinicalNotes}</div>
                      </div>
                    )}
                    {rec.attachments && (() => {
                      try {
                          ...
                          <div className="flex flex-wrap gap-2 pt-2">
                            {atts.map((a, i) => (
                              <a key={i} href={a.url} target="_blank" rel="noreferrer"
                                className="text-xs px-2 py-1 rounded bg-white/10 text-white hover:bg-white/20 border border-white/10">
          ...
          {activeTab === "files" && (
            <div className="space-y-4">
              <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                <div className="text-sm font-semibold text-white mb-3">Subir archivo clínico</div>
                <div className="flex gap-3 items-center">
                  <Select value={uploadCategory} onValueChange={setUploadCategory}>
                    <SelectTrigger className="w-40"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="before">Foto Antes</SelectItem>
                      <SelectItem value="after">Foto Después</SelectItem>
                      <SelectItem value="xray">Radiografía</SelectItem>
                      <SelectItem value="study">Estudio clínico</SelectItem>
                ...
                <div className="text-xs text-muted-foreground mt-2">Formatos: JPG, PNG, PDF. Máx 16MB.</div>
              </div>

              {allAttachments.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">Sin archivos subidos</div>
              ) : (
                <div className="grid grid-cols-2 gap-3">
                  {allAttachments.map((att, i) => (
                    <a key={i} href={att.url} target="_blank" rel="noreferrer"
                      className="bg-white/5 rounded-lg p-3 border border-white/10 hover:bg-white/10 transition-colors">
                      <div className="text-xs text-[#FF6B00] font-semibold mb-1">{categoryLabel[att.category] || att.category}</div>
                      {att.url.match(/\.(jpg|jpeg|png|webp)$/i) ? (
                        <img src={att.url} alt="" className="w-full h-24 object-cover rounded mb-2" />
                      ) : (
                        <div className="w-full h-24 bg-white/5 rounded mb-2 flex items-center justify-center text-3xl">📄</div>
                      )}
                      <div className="text-xs text-muted-foreground">
                        {new Date(att.uploadedAt).toLocaleDateString("es-MX")}
                      </div>
                    </a>
                  ))}
                </div>
...
export default function MedicalAgenda() {
  ...
  const deletePatient = trpc.medical.patients.delete.useMutation({
    onSuccess: () => {
      utils.medical.patients.list.invalidate();
  ...
  const updateAppt = trpc.medical.appointments.update.useMutation({
    onSuccess: () => utils.medical.appointments.list.invalidate(),
  });
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => setShowApptForm(true)}>
              + Nueva cita
            </Button>
            <Button
              className="bg-[#FF6B00] hover:bg-[#e55f00] text-white"
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-white/5 mb-4">
            ...
            <TabsTrigger value="agenda">Agenda de citas</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="patients">
            ...
            {loadingPatients ? (
              ...
              <div className="text-center py-16 text-muted-foreground">
                ...
                <div className="font-semibold">Sin pacientes registrados</div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {filteredPatients.map((patient) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-[#FF6B00]/20 flex items-center justify-center font-bold text-[#FF6B00] text-lg overflow-hidden flex-shrink-0">
                          {patient.photoUrl
                            ? <img src={patient.photoUrl} alt="" className="w-full h-full object-cover" />
                            : `${patient.firstName[0]}${patient.lastName[0]}`
                          }
          ...
          <TabsContent value="agenda">
            <div className="space-y-4">
              ...
              {todayAppts.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-[#FF6B00] mb-2">HOY</h3>
                  <div className="space-y-2">
                    {todayAppts.map((appt) => {
                      const p = patients.find(pt => pt.id === appt.patientId);
                      const st = statusLabel(appt.status);
                      return (
                        <div key={appt.id} className="bg-[#FF6B00]/10 border border-[#FF6B00]/30 rounded-lg p-4 flex items-center gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            {appt.notes && <div className="text-xs text-white/60 mt-1">{appt.notes}</div>}
                          </div>
              ...
              {/* Próximas citas */}
              <div>
                <h3 className="text-sm font-semibold text-muted-foreground mb-2">PRÓXIMAS CITAS</h3>
                {loadingAppts ? (
                  ...
                  <div className="space-y-2">
                    {upcomingAppts.map((appt) => {
                      ...
                      return (
                        <div key={appt.id} className="bg-white/5 border border-white/10 rounded-lg p-4 flex items-center gap-4">
                          <div className="text-center w-16 flex-shrink-0">
                          ...
                          <div className="text-xs text-muted-foreground">{appt.durationMinutes} min</div>
                          <Button
                            size="sm" variant="outline" className="text-xs text-red-400 border-red-400/30"
                            onClick={() => updateAppt.mutate({ id: appt.id, status: "cancelled" })}
                          >
                            Cancelar
                          </Button>

...
type Chargeback = {
  ...
  // Datos del cliente que hizo el contracargo
    ...
    <DashboardLayout>
      ...
      <Dialog open={!!detailCb} onOpenChange={(v) => { if (!v) setDetailCb(null); }}>
        <DialogContent className="max-w-lg">
          ...
          {detailCb && (
            <div className="space-y-4 py-2">
              {/* Cliente que hizo el contracargo */}
              {(detailCb.payerName || detailCb.payerEmail) && (
                <div className="bg-red-50 border border-red-100 rounded-lg p-3 space-y-1.5">
                  <p className="text-xs font-semibold text-red-700 uppercase tracking-wide flex items-center gap-1">
                    <Info className="w-3 h-3" /> Cliente que inició el contracargo
                  </p>
                  {detailCb.payerName && (
                    <div className="flex items-center gap-2 text-sm">
                      <span className="font-medium text-foreground">{detailCb.payerName}</span>
                    </div>
                  )}
                  {detailCb.payerEmail && (
                    <div className="text-sm text-muted-foreground">{detailCb.payerEmail}</div>
                  )}
                  {detailCb.payerPhone && (
                    <div className="text-sm text-muted-foreground">{detailCb.payerPhone}</div>
                  )}
                  {detailCb.operationNumber && (
                    <div className="text-xs text-muted-foreground">Operación: <span className="font-mono">{detailCb.operationNumber}</span></div>
                  )}
                  {detailCb.linkDescription && (
                    <div className="text-xs text-muted-foreground">Cobro: {detailCb.linkDescription}</div>
                  )}
                  {detailCb.stripeDisputeId && (
                    <div className="text-xs text-muted-foreground">Disputa Stripe: <span className="font-mono">{detailCb.stripeDisputeId}</span></div>
                  )}
                  {detailCb.dueBy && (
                    <div className="text-xs text-orange-700 font-medium">Fecha límite respuesta: {new Date(detailCb.dueBy).toLocaleDateString("es-MX")}</div>
                  )}
                </div>
              )}

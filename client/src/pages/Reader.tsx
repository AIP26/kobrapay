    ...
    <DashboardLayout title="Compra tu Lector">
      <div className="p-6 space-y-8">
        ...
        <div className="text-center max-w-2xl mx-auto">
          ...
          <h1 className="text-2xl font-bold text-foreground mb-2">Compra tu Lector KobraPay</h1>
          <p className="text-muted-foreground">Acepta pagos con tarjeta en tu negocio físico. Sin rentas mensuales, solo pagas por transacción exitosa.</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {READERS.map((reader) => (
            ...
            >
              {reader.badge && (
                <div className={`absolute -top-3 left-6 ${reader.badgeColor} text-foreground text-xs font-semibold px-3 py-1 rounded-full`}>
                  {reader.badge}
                </div>
              )}
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-foreground">{reader.name}</h3>
                    <p className="text-sm text-muted-foreground">{reader.subtitle}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-foreground">${reader.price.toLocaleString("es-MX")}</p>
                    <p className="text-xs text-muted-foreground">MXN + IVA</p>
                  </div>
                </div>
                ...
                {selected === reader.id && (
                  <Button
                    className="w-full mt-5 bg-emerald-500 hover:bg-emerald-400 text-foreground"
        ...
        {/* How it works */}
        <div>
          <h2 className="text-lg font-bold text-foreground mb-4 text-center">¿Cómo funciona?</h2>
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              <div key={step} className="bg-gray-50 rounded-xl p-5 text-center">
                <div className="w-10 h-10 bg-emerald-500 text-foreground rounded-full flex items-center justify-center font-bold text-lg mx-auto mb-3">{step}</div>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-foreground">Comprar {selectedReader.name}</h3>
                  <p className="text-sm text-muted-foreground">${selectedReader.price.toLocaleString("es-MX")} MXN + IVA</p>
                </div>
                <button onClick={() => setShowForm(false)} className="text-muted-foreground hover:text-muted-foreground text-xl">×</button>
              </div>
              <div className="p-6 space-y-4">
                ...
                <div>
                  <Label className="text-xs text-muted-foreground mb-1 block">Teléfono de contacto *</Label>
                  <Input placeholder="55 1234 5678" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground mb-1 block">Dirección de envío *</Label>
                  <Input placeholder="Calle, número, colonia" value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label className="text-xs text-muted-foreground mb-1 block">Ciudad / Estado *</Label>
                ...
                <div className="flex gap-3">
                  ...
                  <Button
                    className="flex-1 bg-emerald-500 hover:bg-emerald-400 text-foreground"
                    onClick={handleOrder}

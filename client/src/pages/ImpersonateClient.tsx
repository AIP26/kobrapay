...
function getRoleBadge(role: string) {
  switch (role) {
    ...
    default: return <Badge className="bg-gray-100 text-gray-600 border-gray-200">Usuario</Badge>;
  }
}
    ...
    return (
      <DashboardLayout title="Acceso Denegado">
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-gray-500 max-w-sm">Esta sección es exclusiva para el SuperAdministrador de la plataforma.</p>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="space-y-5">
        ...
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-2xl p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-800 mb-1">Vista de Cliente (Impersonación)</h3>
        ...
        <Card className="border-gray-200 shadow-sm">
          <CardHeader className="border-b border-gray-100 pb-3">
            <CardTitle className="text-base font-semibold text-gray-800 flex items-center gap-2">
              ...
              {users && (
                <span className="text-sm font-normal text-gray-400">({filtered.length} de {users.length})</span>
              )}
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-gray-500 font-medium">{search ? "Sin resultados" : "No hay otros usuarios registrados"}</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-100">
                {filtered.map((u) => (
                  <div key={u.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50/50 transition-colors">
                    ...
                    <Button
                      size="sm"
                      className="bg-purple-600 hover:bg-purple-500 text-white flex-shrink-0 gap-1.5"

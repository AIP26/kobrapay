...
export default function FailedPayments() {
  ...
  return (
    <DashboardLayout title="Cobros Fallidos">
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div className="bg-red-50 border border-red-200 rounded-xl p-3 sm:p-4">
            ...
            <p className="text-xl sm:text-2xl font-bold text-red-700 mt-1">{failed.length}</p>
          </div>
          <div className="bg-orange-50 border border-orange-200 rounded-xl p-3 sm:p-4">
            ...
            <p className="text-lg sm:text-2xl font-bold text-orange-700 mt-1">
              ${totalAmount.toLocaleString("es-MX", { minimumFractionDigits: 0 })}
              <span className="text-xs font-normal ml-1">MXN</span>
            </p>
          </div>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 sm:p-4">
            <p className="text-xs text-amber-600 font-medium uppercase tracking-wide">Error más común</p>
            <p className="text-sm font-bold text-amber-700 mt-1 leading-tight">
        ...
        {isLoading ? (
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            ...
            <div className="hidden lg:grid grid-cols-12 gap-2 px-4 py-2 bg-muted/50 text-xs font-semibold text-muted-foreground uppercase tracking-wide border-b border-border">
                    ...
                    <div className="lg:hidden space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-foreground truncate">{tx.payerName || "—"}</p>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-semibold border ${cat.color}`}>
                          <XCircle className="w-3 h-3" />
                          {cat.label}
                        </span>
                        <p className="text-xs text-muted-foreground">{tx.ownerName || "—"}</p>
                        <p className="text-xs text-muted-foreground/70 ml-auto">
                          {new Date(tx.createdAt).toLocaleDateString("es-MX", { day: "2-digit", month: "short" })}
                        </p>
                      </div>
                      <p className="text-xs text-foreground/80 line-clamp-2">{tx.errorMessage}</p>
                    </div>

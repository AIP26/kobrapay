  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          {isSuperAdmin && (
            <Button
              ...
              className={`border-white/20 text-sm ${showAll ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/40" : "text-gray-400 hover:text-white"}`}
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-[#141c2e] border border-white/10 rounded-xl p-4">
        ...
        <div className="bg-[#141c2e] border border-white/10 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-gray-400">Convertidas</span>
          </div>
        ...
        <div className="bg-[#141c2e] border border-white/10 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-gray-400">Volumen cotizado</span>
          </div>
      ...
      <div className="relative mb-4">
        ...
        <Input
          ...
          className="pl-9 bg-white/5 border-white/10 text-white placeholder:text-gray-500 focus:border-emerald-500"
        />
      </div>

      {/* Tabla */}
      <div className="bg-[#141c2e] border border-white/10 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left px-4 py-3 text-gray-400 font-medium">Prospecto</th>
                {(showAll && isSuperAdmin) && (
                  <th className="text-left px-4 py-3 text-gray-400 font-medium">Asesor</th>
                )}
                <th className="text-left px-4 py-3 text-gray-400 font-medium">Modo</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium">Monto</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium">Tasa KP</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium">Neto</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium">Efectiva</th>
                <th className="text-center px-4 py-3 text-gray-400 font-medium">Estado</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium">Fecha</th>
              </tr>
            </thead>
            <tbody>
              {logsQuery.isLoading ? (
                ...
                <tr>
                  <td colSpan={9} className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-sm">No hay cotizaciones enviadas aún</p>
                    <p className="text-gray-600 text-xs mt-1">Las cotizaciones que envíes desde el simulador aparecerán aquí</p>
                  </td>
                </tr>
              ) : (
                filtered.map((log: any) => (
                  <tr key={log.id} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    <td className="px-4 py-3">
                      <p className="text-white font-medium">{log.prospectName || "—"}</p>
                      <p className="text-gray-500 text-xs">{log.prospectEmail}</p>
                    </td>
                    {(showAll && isSuperAdmin) && (
                      <td className="px-4 py-3 text-gray-300 text-xs">{log.senderName || "—"}</td>
                    )}
                    <td className="px-4 py-3">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-white/10">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={page === 1}
                className="border-white/20 text-gray-400 hover:text-white disabled:opacity-30"
              >
